...
export function ThesisSection({ data }: ThesisSectionProps) {
  ...
  return (
    <div className="border-b border-[#2A2A2A] relative overflow-visible">
      ...
      <div className="h-12 flex items-center px-6 bg-[#111111] select-none border-b border-[#2A2A2A]">
        <div className="flex items-center gap-2">
          <ChevronDown className="w-3.5 h-3.5 text-white/50" />
          <span className="font-semibold text-[14px] text-white/90 uppercase tracking-wider">Thesis</span>
        </div>
      </div>
      ...
      <div className="p-6 bg-[#141414] relative overflow-visible">
        ...
        <div className="space-y-8 text-[14px]">
          ...
          <div className="space-y-4">
            <h3 className="text-[12px] text-white/70 uppercase tracking-wider font-semibold border-l-2 border-white/20 pl-3">
              Supporting Factors
            </h3>
              ...
              <ul className="space-y-4">
                {thesis.factorsFor.map((factor, i) => (
                  <li key={i} className="flex gap-4">
                    <span className="text-white/30 font-mono text-[10px] pt-1 font-semibold">+{i + 1}</span>
          ...
          <div className="space-y-4">
            <h3 className="text-[12px] text-white/80 uppercase tracking-wider font-semibold border-l-2 border-white/40 pl-3">
              Risk Factors
            </h3>
            {thesis.factorsAgainst && thesis.factorsAgainst.length > 0 ? (
              <ul className="space-y-4">
                {thesis.factorsAgainst.map((factor, i) => (
                  <li key={i} className="flex gap-4">
                    <span className="text-white/50 font-mono text-[10px] pt-1 font-semibold">-{i + 1}</span>
                    <div className="space-y-2 flex-1">
                      <p
                        className="text-white/80 cursor-help leading-relaxed"
                        title={factor.point.length > 200 ? factor.point : undefined}
                      >
                        {factor.point}
                      </p>
